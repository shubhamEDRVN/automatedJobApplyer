...
async def main(dry_run: bool):
    ...
    search_queries = [("software engineer intern", "remote")]

    c.execute("SELECT * FROM preferences WHERE profile_id=?", (profiles[0]["id"],))
    pref_row = c.fetchone()
    if pref_row:
        import json
        ...
        # Search for up to 3 role keywords across the first location
        search_queries = [(role, locs[0]) for role in roles[:3]]
    ...
    inserted_count = save_jobs(clean_jobs)
    ...
    for profile in profiles:
        ...
        stats = {"new_found": global_stats["new_found"], "matched": 0, "applied": 0}
        applied_jobs = []
        ...
        success = send_daily_digest(profile, stats, applied_jobs)
        if success:
